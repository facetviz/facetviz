...
export class ColumnSeries extends BaseSeries {
  ...
  override render(ctx: SeriesRenderContext): void {
    ...
    const catScale = (this.horizontal ? ctx.yScale : ctx.xScale) as CategoryScale;
    const valScale: Scale = this.horizontal ? ctx.xScale : ctx.yScale;
    const g = renderer.group({ class: `jchart-series jchart-column ${this.name}` });
  ...
  private drawDataLabel(ctx: SeriesRenderContext, p: Point, rect: { x: number; y: number; width: number; height: number }): void {
    ...
    if (this.horizontal) {
      ...
      else if (pos === 'center') place = { x: rect.x + rect.width / 2, y: cy, anchor: 'middle' };
      else if (pos === 'base') place = { x: rect.x + 4 + d, y: cy, anchor: 'start' };
      else place = { x: end + 4 + d, y: cy, anchor: 'start' }; // outside
    } else {
      const cx = rect.x + rect.width / 2;
      if (pos === 'inside') place = { x: cx, y: rect.y + 12 + d, anchor: 'middle' };
      else if (pos === 'center') place = { x: cx, y: rect.y + rect.height / 2 + 4, anchor: 'middle' };
      else if (pos === 'base') place = { x: cx, y: rect.y + rect.height - 5 - d, anchor: 'middle' };
